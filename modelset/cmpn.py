...
from featureset.atom_featurization import get_atom_fdim, get_bond_fdim
from featureset.tograph import mol2graph
from trainset.nn_utils import index_select_ND, get_activation_function
import math
...
class CMPNEncoder(nn.Module):
    def __init__(self, args: Namespace, atom_fdim: int, bond_fdim: int):
        ...
        self.W_project = nn.Linear(self.hidden_size,self.hidden_size)
        ...
        # self.atten = MoleculePropertyPredictionModel()
        

    def forward(self, step, mol_graph,) -> torch.FloatTensor:

        f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, atom_num = mol_graph.get_components()
...
class CMPN(nn.Module):
    def __init__(self,
        ...
        self.bond_fdim = bond_fdim or get_bond_fdim(args) + self.atom_fdim # * 2
        self.encoder = CMPNEncoder(self.args, self.atom_fdim, self.bond_fdim)

    def forward(self, step,  batch) -> torch.FloatTensor:

        batch = mol2graph(batch, self.args)

        output = self.encoder.forward(step, batch)
        return output
